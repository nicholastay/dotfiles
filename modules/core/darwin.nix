{ self, inputs, ... }:
{
  flake.darwinModules.core = { pkgs, config, lib, ... }:
  let
    homeDir = "/Users/${config.nt.primaryUser}";
  in
  {
    imports = [ ./_common.nix ];
	...
	cleanup    = "none";  # TODO: probably switch to 'zap' eventually
      ...
      casks = [
        # TODO: Consider migrate to nix gui pkgs? Especially say firefox for config
	"firefox"
	"iterm2"
    ...
    system.primaryUser = config.nt.primaryUser;
